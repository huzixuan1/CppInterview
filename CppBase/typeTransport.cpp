...
| **转换类型**      | **安全性**             | **适用场景**                                  | **注意事项**                             |
|--------------------|------------------------|-----------------------------------------------|------------------------------------------|
| `static_cast`      | 编译时检查，较安全      | 基本类型转换、移除 `const`、合法指针类型转换  | 确保转换逻辑正确                         |
| `dynamic_cast`     | 运行时检查，最安全      | 多态类之间的安全指针/引用转换                 | 基类必须含虚函数；转换失败返回 `nullptr` |
| `const_cast`       | 编译时检查，较安全      | 移除或添加 `const`                            | 避免对真正的 `const` 对象进行修改        |
| `reinterpret_cast` | 不检查，最危险         | 位级别转换、不相关类型转换                    | 谨慎使用，易导致未定义行为               |
