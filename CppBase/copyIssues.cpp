...
| **对比点**         | **浅层拷贝**                                         | **深层拷贝**                                  |
|---------------------|------------------------------------------------------|-----------------------------------------------|
| **性能**            | 较高，直接复制指针，无需额外内存分配。               | 较低，需要分配新内存并复制资源内容。          |
| **资源独立性**      | 不独立，多个对象共享同一份资源。                     | 完全独立，每个对象都有自己的资源副本。         |
| **内存管理复杂性**  | 简单，只需管理指针本身。                             | 较复杂，需确保资源正确释放，避免内存泄漏或重复释放。 |
| **适用场景**        | 数据共享、只读数据、高性能要求的场景。               | 需要资源独立、资源隔离，多线程或修改数据的场景。 |
| **副作用**          | 可能导致双重释放问题或数据污染（一个对象修改资源，其他对象受影响）。 | 减少副作用，数据更加安全和隔离。              |


    浅层拷贝 适合性能敏感、不需要数据独立性或资源不由对象管理的场景。
    深层拷贝 适合需要数据独立性、生命周期独立管理的场景，特别是在多线程或复杂内存管理需求中。
