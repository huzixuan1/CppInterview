...
5. 请举一个适配器模式的实际应用场景？
回答重点：
数据库适配器：假设你在开发一个ORM框架，支持多个数据库，如 MySQL 和 PostgreSQL。你可以通过适配器模式，
将不同数据库的查询接口适配为统一的接口，让客户端代码只关注数据库操作，而不关心具体使用的数据库类型。
...
8. 如何使用适配器模式实现跨平台兼容？
回答重点：适配器模式可以在不同的平台间提供接口转换。例如，假设你有一个应用需要支持 Windows 和 macOS 平台的文件读写接口，
但每个平台提供的接口不同。通过适配器模式，你可以为每个平台编写适配器类，使得客户端代码不需要关心底层平台的差异，只需要通过统一的接口进行文件操作。
9. 可以举一个使用适配器模式的代码实例吗？
回答重点：面试官可能要求你通过一个简单的代码示例来演示适配器模式，比如如何将一个两脚插头适配为三脚插头（电源插头示例）或者如何在数据库操作中使用适配器模式来统一接口（ORM 示例）。
你可以展示如何创建目标接口、源接口、适配器类，并且展示如何通过适配器使客户端代码无需更改即可使用不同接口。
10. 适配器模式的缺点有哪些？如何解决？
回答重点：
缺点：
...
总结：
适配器模式是一个常见的设计模式，通常涉及接口转换和适配的问题。面试时，面试官往往会测试你对适配器模式的理解，要求你讲解适配器模式的原理、应用场景、优缺点，
    以及如何在实际项目中使用它。同时，通过代码示例展示如何实现适配器模式也是常见的面试问题。
